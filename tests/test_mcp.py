...
def _settings(*, enabled: bool = True, token: str = "") -> SimpleNamespace:
    return SimpleNamespace(
        ...
        mcp_api_token=token,
        mcp_mount_path="/mcp",
        mcp_server_name="docrag-mcp",
    )


def test_setup_mcp_exposes_only_included_operations() -> None:
    """El servidor MCP publica exactamente las tools permitidas y nada más."""
...
def test_ensure_mcp_access_allows_matching_token(monkeypatch) -> None:
    monkeypatch.setattr(
        mcp_server, "get_settings", lambda: _settings(token="secret")
    )
    # No debe lanzar.
...
def test_ensure_mcp_access_rejects_missing_authorization_header(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        mcp_server, "get_settings", lambda: _settings(token="secret")
...
def test_ensure_mcp_access_rejects_malformed_authorization_header(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        mcp_server, "get_settings", lambda: _settings(token="secret")
...
def test_ensure_mcp_access_open_without_token(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        mcp_server, "get_settings", lambda: _settings(token="")
    )
...
def test_ensure_mcp_access_returns_404_when_disabled(monkeypatch) -> None:
    monkeypatch.setattr(
        mcp_server,
        "get_settings",
        lambda: _settings(enabled=False, token="secret"),
    )
    with pytest.raises(HTTPException) as exc:
