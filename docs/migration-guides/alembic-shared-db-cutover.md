...
Ejecutar Fase 6 del cutover cuando KDB-RAG-Docs y KDB-RAG-Repo comparten la misma base Postgres.
...
## Fase 6.2: Poblar/Actualizar tabla Alembic de Docs
...
# 3) Validacion de bootstrap runtime
.\.venv\Scripts\python -c "from coderag.core.settings import SETTINGS; from coderag.storage.postgres_startup import ensure_postgres_schema_ready; import json; print(json.dumps(ensure_postgres_schema_ready(SETTINGS, force=True), ensure_ascii=True))"
```

## Fase 6.3: Verificacion cruzada

Comprobar que Docs no usa la tabla de Repo:
...
- `alembic_version_repo` contiene solo revisiones de repo.
- `ensure_postgres_schema_ready(..., force=True)` finaliza OK.
...
3. Repetir validacion de `current` y `ensure_postgres_schema_ready`.
...
- Al menos 2 ciclos de despliegue de Docs y Repo sin colisiones Alembic.
...
- `alembic_version_docs` y `alembic_version_repo` existen y reflejan los heads esperados.
- Docs y Repo arrancan y validan migraciones sin errores en la ventana de observacion.
